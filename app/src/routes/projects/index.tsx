...
import { z } from 'zod';
import { useSearchProjects, Project } from 'src/hooks/useProjects';
...
function Projects() {
  ...
  const canCreate = hasPermission(user?.permissions, Resources.Project, ProjectPermission.create);
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('name', {
        ...
        cell: (info) => (
          <Text
            ...
            params={{ projectId: info.row.original.id } as any}
            fw={500}
            size="sm"
            c="appBlue"
      ...
      columnHelper.accessor('organization.name', {
        header: m.entities_organization(),
  ...
  return (
    <Stack gap="xl">
      <PageHeader
        title={m.common_navigation_projects()}
        subTitle={m.pages_projects_viewprojects_subtitle()}
      >
        <Button
          component={Link}
          to="/projects/create"
          ...
          disabled={!canCreate}
        >
          {m.pages_projects_viewprojects_createproject()}
        </Button>
      </PageHeader>
        ...
        <Box p="md" style={{ borderTop: '1px solid var(--mantine-color-default-border)' }}>
          <Group justify="space-between">
            <Text size="sm" c="dimmed">
              Showing {tableData.length} of {data?.totalCount || 0} projects
            </Text>
            <Group gap="xs">
              <Button
                variant="default"
