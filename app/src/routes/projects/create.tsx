...
function CreateProject() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        const result = await createProjectAction({ data: value as any });
        notifications.show({
          title: m.common_created(),
          message: m.messages_success_projectcreated({ name: result.name }),
          color: 'green',
          icon: <HiOutlineCheck />,
        });
        navigate({ to: '/projects/$projectId', params: { projectId: result.id } as any });
      } catch (error) {
        notifications.show({
          title: m.messages_error_error(),
          message: m.messages_error_projectcreatefailed(),
          color: 'red',
        });
      }
    },
  ...
  const breadcrumbs = [
    ...
    { title: m.pages_projects_createproject_title(), to: '/projects/create' },
  ].map((item, index) => (
  ...
  return (
    <Stack gap="xl" style={{ position: 'relative', minHeight: '400px' }}>
      <Box>
        ...
        <PageHeader
          title={m.pages_projects_createproject_title()}
          subTitle={m.pages_projects_createproject_subtitle()}
        >
          <Button 
            component={Link} 
            to="/projects"
            ...
            <form.Field
              ...
              validators={{
                onBlur: ({ value }: { value: string }) => {
                  const res = projectSchema.shape.organizationId.safeParse(value);
              ...
              children={(field) => (
                <Select
                  ...
                  onChange={(value) => {
                    ...
                    form.setFieldValue('userIds', []); // Reset users when org changes
                  }}
