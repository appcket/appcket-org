...
function Teams() {
  ...
  const canCreate = hasPermission(user?.permissions, Resources.Team, TeamPermission.create);
  ...
  const { data, isLoading, isFetching, isPlaceholderData } = useSearchTeams(
    search.search || '',
    pageSize,
    search.cursor || null,
    orderByString,
      ...
      columnHelper.accessor('name', {
        ...
        cell: (info) => (
          <Text
            component={Link}
            to="/teams/$teamId"
            params={{ teamId: info.row.original.id } as any}
            fw={500}
            size="sm"
            c="appBlue"
  ...
  return (
    <Stack gap="xl">
      <PageHeader title={m.common_navigation_teams()} subTitle={m.pages_teams_viewteams_subtitle()}>
        <Button
          component={Link}
          to="/teams/create"
          ...
          disabled={!canCreate}
        >
          {m.pages_teams_viewteams_createteam()}
        </Button>
      </PageHeader>
        ...
        <Box p="md" style={{ borderTop: '1px solid var(--mantine-color-default-border)' }}>
          <Group justify="space-between">
            <Text size="sm" c="dimmed">
              Showing {tableData.length} of {data?.totalCount || 0} teams
            </Text>
            <Group gap="xs">
              <Button
                variant="default"
