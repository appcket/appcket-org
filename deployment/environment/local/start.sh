...
helm package "${REPO_ROOT}/deployment/helm" -d "${REPO_ROOT}"

# Start project with helm chart
echo '---------------------'
echo "Starting ${PROJECT_MACHINE_NAME}..."
helm install ${PROJECT_MACHINE_NAME} "${REPO_ROOT}/${PROJECT_MACHINE_NAME}-0.1.0.tgz" -n ${PROJECT_MACHINE_NAME} -f "${REPO_ROOT}/deployment/helm/values-local.yaml"
